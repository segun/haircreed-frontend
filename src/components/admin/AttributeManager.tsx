...
export default function AttributeManager({ category, onUpdate, onDeleteCategory }: AttributeManagerProps) {
    ...
    return (
        <>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md relative">
                ...
                <h3 className="text-lg font-semibold text-zinc-800 mb-4">{category.title}</h3>
                
                {error && <p className="text-sm text-red-500 bg-red-100 p-2 rounded-md mb-4">{error}</p>}
                ...
                <div className="flex items-center space-x-2 mb-4">
                    ...
                    {editingItem ? (
                        <>
                            <button onClick={handleUpdateItem} className="px-4 py-2 text-sm font-medium text-white bg-zinc-600 hover:bg-zinc-700 rounded-md">Update</button>
                            <button onClick={() => setEditingItem(null)} className="px-4 py-2 text-sm font-medium text-zinc-700 bg-zinc-100 hover:bg-zinc-200 rounded-md">Cancel</button>
                        </>
                    ) : (
                        <button onClick={handleAddItem} className="p-2 text-white bg-zinc-800 hover:bg-zinc-900 rounded-full">
                            <PlusCircle size={20} />
                        </button>
                    )}
                </div>
                ...
                <ul className="space-y-2 h-48 overflow-y-auto">
                    {category.items.length > 0 ? category.items.map(item => (
                        <li key={item.id} className="flex items-center justify-between p-2 rounded-md hover:bg-zinc-50">
                            <span className="text-zinc-700">{item.name}</span>
                            <div className="flex items-center space-x-3">
                                <button onClick={() => setEditingItem(item)} className="text-zinc-500 hover:text-zinc-700">
                                    <Edit2 size={16} />
                                </button>
                        ...
                        <p className="text-sm text-zinc-500 text-center py-4">No items yet.</p>
                    )}
                </ul>
            </div>
        </>
